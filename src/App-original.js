...
const App = () => {
  ...
  const showNotification = useCallback((title, body) => {
    if (notificationPermission === 'granted') {
      new Notification(title, { body, icon: `${process.env.PUBLIC_URL}/favicon.ico` });
    }
  ...
  return (
    <div className={`app ${theme.includes('Liquid Glass') ? `liquid-glass-theme ${theme.replace(/\s+/g, '-').toLowerCase()}` : ''}`} style={{
      ...
      <div className="container">
        <header>
          <h1><img src={`${process.env.PUBLIC_URL}/pomodoro-icon.png`} alt="Pomodoro" className="pomodoro-icon" /> Pomodoro Pro</h1>
